...
def calc_yn00(
    ...
    with Progress() as progress:
        task = progress.add_task("YNOO running.", total=len(args))
    ...
    ezwgd.console.log(f'YN00 finished in {used_seconds//60:.0f} min {used_seconds%60:.1f} sec.')
    return results
...
def calc_codeml(
    ...
    with Progress() as progress:
        ...
        with Pool(processes=processes_num) as pool:
            async_results = [
                pool.apply_async(_calc_codeml_tasks, a, callback=_cb)
                for a in args
            ]
            results = [ar.get() for ar in async_results]

    used_seconds = time.time() - codeml_start
    ezwgd.console.log(f'CODEML pairwise dn/ds calculation finished in {used_seconds//60:.0f} min {used_seconds%60:.1f} sec.')
    return results
